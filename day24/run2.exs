defmodule Day24 do
  def solution(path) do
    ...
    inputs
    |> Enum.reduce([%{}, %{}], fn {name, value}, [x, y] ->
      if String.starts_with?(name, "x") do
        [Map.put(x, name, value), y]
      else
        [x, Map.put(y, name, value)]
      end
    end)
    |> Enum.map(fn map ->
      map
    ...
    |> Enum.zip()
    |> IO.inspect()
    |> Enum.map(fn {a, b} -> and_gate(a, b) end)
    |> IO.inspect()
...
Day24.solution("input_test2.txt")
